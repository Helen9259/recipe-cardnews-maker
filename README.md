...
`retryDelay`만큼(없으면 지수 백오프로) 자동 대기 후 최대 5회까지 재시도합니다. 이 큐는 같은 Node
프로세스 안에서만 유지되는 메모리 상태라 서버리스 인스턴스가 새로 뜨면 초기화되며, 완전한 전역
레이트리밋은 아니라는 점을 감안해주세요.

이 때문에 추천 레시피(영상 5개 구조화 + 카테고리 분류 1회 = Gemini 호출 6회)는 최소 1분 20초 이상
걸릴 수 있습니다. 카테고리 분류는 영상마다 따로 호출하지 않고 한 번의 요청에 모아서 보냅니다.

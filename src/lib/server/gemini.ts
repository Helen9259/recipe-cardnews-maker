...
const ILLUSTRATION_KEYWORDS_SCHEMA = {
  type: "object",
  properties: {
    keywords: {
      type: "array",
...
export interface IllustrationKeyword {
  step: number;
  keyword: string;
}

/**
 * 레시피 재료 목록 + 전체 조리 단계를 한 번에 Gemini에 보내서, 각 단계 삽화의 핵심
 * 오브젝트/재료를 영어 단어로 뽑는다. 단계가 몇 개든 이 호출은 딱 1번만 발생한다.
 * (a) 한글 재료명을 영어로 번역, (b) 재료 목록 순서가 아니라 문맥상 그 단계를
 * 대표하는 오브젝트를 골라서 단계끼리 겹치지 않게 하는 두 가지 목적을 함께 처리한다.
 */
export async function extractIllustrationKeywords(
  ingredients: string[],
  steps: string[]
): Promise<IllustrationKeyword[]> {
  if (steps.length === 0) return [];

  const prompt = [
    "너는 요리 삽화 감독이야. 아래 재료 목록과 조리 단계를 보고, 각 단계의 삽화에 그릴",
    "핵심 오브젝트/재료를 영어 단어(또는 짧은 구)로 하나씩 뽑아줘.",
    "",
    "규칙:",
    "- 재료 목록에 나온 순서가 아니라, 그 단계 문장에서 시각적으로 가장 중요한 오브젝트를 문맥으로 판단해서 골라줘",
    "- 예를 들어 어떤 단계에서 이미 '닭'을 다뤘다면, 다음 단계에서 또 닭이 언급되더라도 겹치지 않게",
    "  '기름'이나 '팬 위의 닭'처럼 그 단계만의 장면을 대표하는 걸로 골라줘 (전체 단계를 한 번에 보고 판단)",
    "- 재료 이름이 아니어도(불, 물, 그릇 등) 그 단계를 대표하는 오브젝트면 괜찮아",
    "- keyword는 반드시 영어 단어/짧은 구로 번역해서 작성 (예: '다진 마늘' → 'minced garlic')",
    "- keywords 배열은 steps 배열과 같은 개수로, step은 1부터 시작하는 순번",
    "",
    `재료 목록: ${ingredients.join(", ")}`,
    "",
    "조리 단계:",
    ...steps.map((text, idx) => `${idx + 1}. ${text}`),
  ].join("\n");

  const parsed = (await callGeminiJSON([{ text: prompt }], ILLUSTRATION_KEYWORDS_SCHEMA)) as {
    keywords?: IllustrationKeyword[];
  };
